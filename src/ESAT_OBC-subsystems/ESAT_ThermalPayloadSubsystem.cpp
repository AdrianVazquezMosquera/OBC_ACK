...
void ESAT_ThermalPayloadSubsystemClass::handleTelecommand(ESAT_CCSDSPacket& packet)
{
  ...
  switch (secondaryHeader.packetIdentifier)
  {
    case 0:
      ...
      savePacketIdentifier(secondaryHeader.packetIdentifier);
      break;
    case 1:
...
byte ESAT_ThermalPayloadSubsystemClass::savePacketIdentifier(byte packetIdentifier)
{
  return datum = packetIdentifier;
}
...
boolean ESAT_ThermalPayloadSubsystemClass::readTelemetry(ESAT_CCSDSPacket& packet)
{
  if (pendingTelemetryPackets > 0)
  {
    ...
    packet.writeByte(datum);

    // The ACK identifier is reseted to standby
    byte standby = 0x10;
    savePacketIdentifier(standby);
