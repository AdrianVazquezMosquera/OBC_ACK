...
void ESAT_OBCSubsystemClass::beginTelemetry()
{
  ...
  addTelemetry(ESAT_OBCACKTelemetry);
  enableTelemetry(ESAT_OBCACKTelemetry.packetIdentifier());
}
...
void ESAT_OBCSubsystemClass::handleTelecommand(ESAT_CCSDSPacket &packet)
{
  ...
  {
    ESAT_OBCACKTelemetry.saveSecondaryHeader(packet);
    bool ACKAvailable = true;
    ESAT_OBCACKTelemetry.makeAvailable(ACKAvailable);
  }
}
